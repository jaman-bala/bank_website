...
class Organizations(models.Model):
    ...
    class Meta:
        verbose_name = 'Орг форма'
        verbose_name_plural = 'Орг формы'
...
class Create(models.Model):
    ...
    inn = models.DecimalField("ИНН", max_length=255, unique=True)
    ...
    supervisor_phone = models.DecimalField("Номер телефона", max_length=16)
    ...
    phone = models.DecimalField("Номер телефона", max_length=16)
    email = models.EmailField("Почта")

    requested_amount = models.CharField("Запрашиваемая сумма", max_length=255)
    credit_term = models.CharField("Срок кредита", max_length=255)
